...
import vonnie.vonniestest.config.OregenConfig;
    ...
    public void generateWorld(Random random, int chunkX, int chunkZ, World world, boolean newGen) {
        ...
        if (world.provider.getDimension() == DimensionType.OVERWORLD.getId()) {
            if (OregenConfig.GENERATE_OVERWORLD) {
                addOreSpawn(ModBlocks.blockTurquoiseOre, (byte) OreType.ORE_OVERWORLD.ordinal(), Blocks.STONE, world, random, chunkX * 16, chunkZ * 16,
        ...
        if (world.provider.getDimension() == DimensionType.NETHER.getId()) {
            if (OregenConfig.GENERATE_NETHER) {
                addOreSpawn(ModBlocks.blockTurquoiseOre, (byte) OreType.ORE_NETHER.ordinal(), Blocks.NETHERRACK, world, random, chunkX * 16, chunkZ * 16,
                        OregenConfig.MIN_VEIN_SIZE, OregenConfig.MAX_VEIN_SIZE, OregenConfig.CHANCES_TO_SPAWN, OregenConfig.MIN_Y, OregenConfig.MAX_Y);
            }
        }
        if (world.provider.getDimension() == DimensionType.THE_END.getId()) {
            if (OregenConfig.GENERATE_END) {
                addOreSpawn(ModBlocks.blockTurquoiseOre, (byte) OreType.ORE_END.ordinal(), Blocks.END_STONE, world, random, chunkX * 16, chunkZ * 16,
                        OregenConfig.MIN_VEIN_SIZE, OregenConfig.MAX_VEIN_SIZE, OregenConfig.CHANCES_TO_SPAWN, OregenConfig.MIN_Y, OregenConfig.MAX_Y);
            }
        }
        if (!newGen) {
        ...
        if (tag != null) {
            ...
            if (generated) {
                if (OregenConfig.VERBOSE) {
                    vonniestest.logger.log(Level.DEBUG, "Queuing Retrogen for chunk: " + coord.toString() + ".");
                }
                regen = true;
            }
        } else {
